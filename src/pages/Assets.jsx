        ...
        <div className="max-w-6xl mx-auto space-y-6">
            ...
            {assets.length > 0 && (
                <Card className="p-4">
                    <div className="relative">
                        ...
                        <input
                            type="text"
                            placeholder="Varlık adı veya türüne göre ara..."
                            value={searchTerm}
            ...
            {loading ? (
                ...
                <Card className="text-center py-16 border-dashed border-2 border-gray-200">
                    ...
                    <h3 className="text-xl font-semibold text-gray-700 mb-2">Henüz mal varlığı kaydınız bulunmuyor.</h3>
                    <p className="text-gray-500 mb-6">Gayrimenkul, araç ve diğer varlıklarınızı ekleyerek kayıt altına alın.</p>
                    <Button onClick={() => handleOpenModal()} variant="primary" className="inline-flex items-center gap-2 bg-yellow-600 hover:bg-yellow-700">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredAssets.map(asset => (
                        <Card key={asset.id} className="hover:shadow-lg transition duration-300 group">
                            <div className="p-6">
                                <div className="flex justify-between items-start mb-4">
                                    <div className={`px-3 py-1.5 rounded-lg text-sm font-medium flex items-center gap-2 ${typeColorMap[asset.type] || typeColorMap.diger}`}>
                                        {getAssetIcon(asset.type)}
                                        {getAssetLabel(asset.type)}
                                    </div>
                                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button onClick={() => handleOpenModal(asset)} className="p-2 min-h-[44px] min-w-[44px] flex items-center justify-center text-blue-500 hover:bg-blue-50 rounded"><FaEdit /></button>
                                        <button onClick={() => setDeleteId(asset.id)} className="p-2 min-h-[44px] min-w-[44px] flex items-center justify-center text-red-500 hover:bg-red-50 rounded"><FaTrash /></button>
                                    </div>
                                </div>
                                <h3 className="font-bold text-lg text-gray-800 mb-2">{asset.description}</h3>
                                <div className="text-2xl font-bold text-gray-800 mb-2">{formatCurrency(asset.value)}</div>
                                {asset.notes && (
                                    <p className="text-sm text-gray-500 mt-3 pt-3 border-t border-gray-100">{asset.notes}</p>
                                )}
                            </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <select
                            ...
                            className="w-full px-4 py-2.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-300 bg-white min-h-[44px]"
                        >
                            {ASSET_TYPES.map(t => (
                    ...
                    <Input label="Açıklama" value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} required placeholder="Örn: Ankara Çankaya'daki daire" />
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            className="w-full px-4 py-2.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-300 min-h-[80px] resize-y"
                        ></textarea>
                    </div>
            ...
            <Modal isOpen={!!deleteId} onClose={() => setDeleteId(null)} title="Emin misiniz?">
                <div className="text-center">
                    ...
                    <p className="text-gray-500 mb-6">Bu varlığı silmek istediğinize emin misiniz? Bu işlem geri alınamaz.</p>
